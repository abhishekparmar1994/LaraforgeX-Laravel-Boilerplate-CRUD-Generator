...
@section('title', 'LaraforgeX — Setting Module')
...
<nav class="flex items-center gap-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
  ...
  <span class="text-slate-700">Setting</span>
</nav>
@endsection
...
<div class="space-y-5 font-sans">
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    <div>
      <h2 class="text-xl font-bold text-slate-900">Setting Management</h2>
      <p class="text-xs text-slate-500 mt-0.5 font-medium">Manage, search, filter and export Setting records.</p>
    </div>
...
<div id="modal-setting" class="fixed inset-0 z-50 overflow-y-auto hidden">
  <div class="flex items-center justify-center min-h-screen px-4 py-6">
    ...
    <form id="form-setting" class="relative bg-white border border-slate-200 rounded-2xl w-full max-w-lg p-6 shadow-2xl space-y-5 font-sans">
      <input type="hidden" id="record-id">
      <h3 class="font-bold text-lg text-slate-900" id="modal-title">Create Setting</h3>
      ...
      <div class="flex gap-3 pt-1">
        ...
        <button type="submit" class="w-1/2 py-2.5 rounded-lg bg-brand-600 text-white text-xs font-bold">Save Setting</button>
      </div>
    </form>
  </div>
</div>
...
$(document).ready(function() {
  const table = new AdminTable({
    container: '#datatable-setting',
    columns: [
      { key: 'id', label: 'Id', sortable: false },
      { key: 'key', label: 'Key', sortable: true },
      { key: 'value', label: 'Value', sortable: false },
      { key: 'group', label: 'Group', sortable: true },
      { key: 'is_encrypted', label: 'Is Encrypted', sortable: false },
