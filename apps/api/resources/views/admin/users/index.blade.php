...
@section('content')
  <div class="space-y-5 font-sans">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div>
        ...
        <p class="text-xs text-slate-500 mt-0.5 font-medium">Manage profiles, access control mappings, and activation
          states.</p>
      </div>
  ...
  <div id="modal-user" class="fixed inset-0 z-50 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen px-4 py-6">
      ...
      <form id="form-user"
        class="relative bg-white border border-slate-200 rounded-2xl w-full max-w-lg p-6 shadow-2xl space-y-5 font-sans">
        <input type="hidden" id="user-edit-id">
        <h3 class="font-bold text-lg text-slate-900" id="user-modal-title">Create New Profile</h3>
        <div class="space-y-4">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="space-y-1">
              <label for="user-name" class="text-xs font-semibold uppercase tracking-wider text-slate-500">Full
                Name</label>
            ...
            <div class="space-y-1">
              <label for="user-email" class="text-xs font-semibold uppercase tracking-wider text-slate-500">Email
                Address</label>
          ...
          <div class="space-y-1" id="user-password-container">
            <label for="user-password"
              class="text-xs font-semibold uppercase tracking-wider text-slate-500">Password</label>
        ...
        <div class="flex gap-3 pt-1">
          <button type="button"
            class="close-modal w-1/2 py-2.5 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-500 text-xs font-semibold transition">Cancel</button>
          <button type="submit"
            class="w-1/2 py-2.5 rounded-lg bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold transition">Save
            User</button>
        </div>
      </form>
    </div>
...
@section('scripts')
  <script>
    $(document).ready(function () {
      ...
      // Hide roles field container if they lack roles permission
      if (!canViewRoles) {
        ...
        fetch: async () => {
          ...
          if (canViewRoles) {
            const [usersRes, rolesRes] = await Promise.all([
              ...
              axios.get('/roles').catch(() => ({ data: { data: { roles: [] } } })),
            ]);
            rolesList = (rolesRes.data && rolesRes.data.data && rolesRes.data.data.roles) ? rolesRes.data.data.roles : [];
