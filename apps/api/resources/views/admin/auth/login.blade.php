    ...
    <div class="bg-white/95 border border-slate-200/80 p-8 sm:p-10 rounded-2xl shadow-xl shadow-slate-200/50 space-y-8">
      ...
      <div class="text-center space-y-3.5">
        ...
        <div class="space-y-1">
          ...
          <p id="login-subtitle" class="text-xs text-slate-400 font-bold uppercase tracking-wider">Sign in to your Laraforge<span class="text-brand-600">X</span> core panel</p>
        </div>
      </div>
      ...
      <form id="login-form" class="space-y-5">
        ...
        <div class="space-y-1.5 password-container">
          ...
          <div class="relative">
            ...
            <input id="password" name="password" type="password" required 
              ...
              placeholder="••••••••••••">
            <button type="button" onclick="togglePasswordVisibility('password', 'toggle-password-icon')" class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-slate-600 transition cursor-pointer border-0 bg-transparent">
        ...
        <div class="flex items-center justify-between text-xs font-semibold">
          <label class="flex items-center text-slate-500 cursor-pointer select-none">
            <input type="checkbox" id="remember" class="mr-2 rounded border-slate-200 bg-white text-brand-600 focus:ring-0">
            Remember me
          </label>
          <div class="flex gap-2">
            <a href="/admin/forgot-password" class="text-slate-400 hover:text-slate-600 transition">Forgot Password?</a>
            <span class="text-slate-200">|</span>
            <a href="javascript:void(0)" id="toggle-login-mode" class="text-brand-600 hover:text-brand-500 transition">Magic Link</a>
          </div>
        </div>
      ...
      <div id="magic-link-block" class="hidden space-y-4 pt-5 border-t border-slate-100">
        <div class="p-3.5 bg-brand-50/80 border border-brand-100/50 text-xs text-brand-700 rounded-xl font-semibold">
          ...
          Since mail delivery depends on local credentials, the generated magic link will be displayed below for direct access.
        </div>
        <div id="magic-link-display" class="p-3.5 bg-slate-50 border border-slate-200 rounded-xl font-mono text-xs text-slate-600 break-all select-all">
          Generating link...
        </div>
        <a id="magic-link-anchor" href="#" class="block text-center py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs transition border-0">
  ...
  <div class="flex items-center justify-center min-h-screen px-4">
    ...
    <div class="relative bg-white border border-slate-200 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-5">
      <div class="text-center space-y-2">
        <div class="h-12 w-12 rounded-full bg-brand-50 border border-brand-100 flex items-center justify-center text-brand-600 text-xl mx-auto">
          <i class="fa-solid fa-shield-halved"></i>
        </div>
        <h3 class="font-bold text-lg text-slate-900">Two-Factor Authentication</h3>
        <p class="text-xs text-slate-500 font-medium">Please enter the 6-digit verification code from Google Authenticator.</p>
      </div>

      <form id="form-2fa" class="space-y-4">
        <div class="space-y-1">
          <input id="code-2fa" type="text" maxlength="6" pattern="[0-9]{6}" required class="w-full text-center tracking-widest text-lg font-mono bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-slate-900 focus:outline-none focus:border-brand-500 placeholder-slate-300 focus:placeholder-transparent" placeholder="000000" autocomplete="one-time-code">
        </div>

        <div class="flex gap-3">
          <button type="button" id="close-2fa" class="w-1/2 py-2.5 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-500 text-xs font-semibold transition cursor-pointer">
            Cancel
          </button>
          <button type="submit" class="w-1/2 py-2.5 rounded-lg bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold transition cursor-pointer border-0">
            Verify & Confirm
          </button>
        </div>
      </form>
