...
@section('title', 'LaraforgeX — Database Studio & Table Manager')
...
@section('breadcrumbs')
  <nav class="flex items-center gap-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
    ...
    <span class="text-slate-700" data-i18n="database_manager">Database Studio</span>
  </nav>
@endsection
    ...
    <div class="theme-hero-banner bg-gradient-to-r from-brand-900 via-brand-700 to-indigo-800 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
      ...
      <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2.5 shrink-0 flex-wrap">
          <a href="/admin/database-manager/console"
          ...
          <a href="/admin/database-manager/create"
      ...
      $.ajax({
        ...
        success: function (res) {
          if (res.success) {
            ...
            $('#metric-db-name').text((data.database_name || 'MySQL').toUpperCase());
        ...
        html += `
          <tr class="hover:bg-slate-50/80 transition">
            ...
            <td class="px-5 py-3.5 font-mono font-bold text-slate-900">
              <a href="/admin/database-manager/manage/${t.name}" class="hover:text-brand-600 hover:underline border-0 bg-transparent cursor-pointer flex items-center gap-2 text-left text-slate-900 no-underline">
            ...
            <td class="px-4 py-3.5 text-right font-mono font-bold text-slate-700">${t.table_rows.toLocaleString()}</td>
            ...
            <td class="px-5 py-3.5 text-right space-x-1.5 whitespace-nowrap">
              <a href="/admin/database-manager/manage/${t.name}" class="px-2.5 py-1 rounded-lg bg-brand-50 hover:bg-brand-100 text-brand-600 font-bold text-[11px] transition border border-brand-200 inline-flex items-center gap-1 no-underline">
                <i class="fa-solid fa-sliders text-[10px]"></i> Manage
              </a>
              <a href="/admin/database-manager/manage/${t.name}?tab=data" class="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-[11px] transition border border-slate-200 inline-flex items-center gap-1 no-underline">
    ...
    function executeBulkTablesAction(action) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: '/api/v1/database-manager/bulk-action',
            type: 'POST',
