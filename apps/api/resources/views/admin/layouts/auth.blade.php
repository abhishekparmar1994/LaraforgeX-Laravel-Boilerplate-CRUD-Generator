...
<head>
  ...
  <script>
    // Axios defaults
      ...
      let container = document.getElementById('laraforgex-toast-container');
      if (!container) {
        container = document.createElement('div');
      container.id = 'laraforgex-toast-container';
      document.body.appendChild(container);
      }
      container.className = `pos-${activePos}`;
      ...
      toastEl.className = `lf-toast-card theme-${activeTheme} lf-toast-${toastType}`;
    ...
    <div class="lf-toast-progress-track">
      ...
      const closeBtn = toastEl.querySelector('.lf-toast-close');
      closeBtn.addEventListener('click', () => removeToast(toastEl));

      if (options.actions && Array.isArray(options.actions)) {
        const btns = toastEl.querySelectorAll('.lf-toast-action-btn');
        btns.forEach(btn => {
